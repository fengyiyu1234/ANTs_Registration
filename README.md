...
The interactive section mask editor and its session tests live in the sibling
`Registration_toolkit` repository. From that repository, run:
...
You can also select `mode: section2d` in
`Registration_toolkit/configs/paint_mask.yaml` and run `python paint_mask.py`.
The exported full-resolution `*_tissue.tif` and `*_damage.tif` files feed
this repository's `sections[].tissue_mask` and `damage_mask` inputs.
Region labels are saved and restored by the editor but are not consumed by
the current 2D registration pipeline.
