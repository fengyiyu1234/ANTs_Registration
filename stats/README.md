...
`Volume` / `RelativeVolume` 与细胞类无关，每个区只检验一次，`class_name` 记为 `region`
（count 门槛用该区全部 base 类细胞之和）。
...
## 固定图谱小块：`block_stats.py`

逐区密度的分母是**样本空间**的体积，所以清透带来的整体缩放全在里面。这套数据里
它是量出来的，不是假想的：全脑体积 49.1–82.4 mm3，level 2/3 的 28 个结构里有 26 个
在一组里小 13–23%，而 RelativeVolume 是平的。固定在**图谱空间**的小块在每个样本里
体积都一样，除以它不改变任何统计量，指标变成"一块固定解剖领地里有多少细胞"。

块由 `Registration_toolkit/tools/pick_blocks.py` 产生，这里只读那份 `blocks.json`。
块的位置必须在看到分组结果之前定死。

```bash
python -m stats.block_stats --config stats/configs/tsc_blocks.yaml
python -m stats.block_stats --config ... --no-figures     # 只要表
python -m stats.plot_blocks --config ... --metrics Count,Share   # 只重画图
```
...
| family | 是什么 | 能说什么 |
|---|---|---|
| `Primary` | 每个样本先对所有块取平均，再做 3v3 | 唯一有像样 p 值的一层，这才是分析 |
| `Per_Block` / `Redistribution` | 逐块检验，class 内 BH | 探索性。说"在哪儿"，而且只对 Primary 已经支持的结果说 |
| `Heterogeneity` + 梯度 | 效应到底有没有位置结构 | 见下 |

块不增加 n。逐块检验永远是 3v3，块在进检验之前已经被平均进它的样本里，任何地方都
没有把块当重复。

### 位置不一样，所以可以问"细胞是不是换地方了"

块的平均值看不见迁移：细胞从一块搬到另一块，平均值一点不动。所以有 `Share` ——
一块的计数除以**同一个样本**在所有块里的总数。注射量、清透、导板覆盖多少，这些
逐样本的"量级"项全在分母里，除掉之后剩下的只有位置。

三样工具，从粗到细：

1. **`I2` / Cochran's Q** —— 效应有没有位置结构。I2 接近 0 就是 28 个块只是同一个
   数的噪声副本，这种情况下"效应在第 17 号块"说的是哪个块抽到了好签，不是解剖。
2. **沿轴的梯度（Spearman ρ vs A-P / D-V / 距中线）** —— 迁移是梯度：细胞从一头
   离开，到另一头去。一个轴一个检验，而不是一个块一个检验，这是这个 n 下唯一还有
   一点把握度的空间问题。
3. **`Redistribution_Global`** —— 两组平均 share 向量之间的 total variation 距离，
   直接读作"要让两组对上，得有多大比例的细胞换块"，对着**精确置换**零分布。

### 两个 p 值是错的，所以都被置换替掉了

Q 的 chi2 p 假设逐块的 SE 是**已知的**；这里每个 SE 只有 2 个自由度，某个块碰巧抽到
小 SD 就会拿到巨大的权重，Q 随之膨胀。Spearman 自己的 p 是反过来的毛病：相邻块只隔
600 µm，明显相关，它的自由度是编的。

把**分组标签**打乱能一次修好两个：块的位置和块的噪声原地不动，只有对比被重新分配，
这正是问题真正要问的零假设。代价是 3v3 到处都有的那个下限——六个样本只有 10 种不同
的分法，**p 最小只能到 0.1**，所以输出的是排名不是显著性。表里 `p_Q_perm`、
`p_<轴>_perm`、`p_permutation` 就是它们，左边那列 chi2 / Spearman 的 p 留着只是为了
让人看见差多少。

这条能救回来的判断不止一个：`glia_MADM_Sox9` 沿距中线方向的 Spearman p 是 0.001，
置换之后是 0.2 —— 那个看起来很强的梯度大部分是空间自相关。

### 逐块成像质量：`Block_QC`

图谱分母不会因为组织没了就变小，所以块里真缺一块组织，读出来就是真的减少。**brain
mask 的覆盖率查不出这件事**：它是个包络，在每个样本的每个块上都读 1.00，包括 s18 那
两个空了五分之一的块。

能查出来的是 warped 的**强度图**。撕裂、气泡、采到样本边缘之外，都表现为一片近零的
体素。逐块报两个数：

| 列 | 含义 |
|---|---|
| `dark_frac` | 块内低于 `dark_rel` × **该样本自己**块内组织中位强度的体积比例 |
| `dark_cc_frac` | 其中**最大连通**暗区的比例。和 `dark_frac` 接近 = 一个洞；小得多 = 散点噪声，更可能是暗组织不是破损 |

阈值是相对的，因为 warped 图逐样本归一化（这六只的中位强度 0.088–0.152）。**别用
Otsu**，它在 light-sheet 上只圈亮细胞、把其余组织当背景。

这个量在本数据上是双峰的：干净的块**恰好 0.0%**，有问题的块几个百分点起，中间没有
东西，所以切在哪儿几乎不影响判定。当前配置（`dark_rel: 0.25`, `dark_max: 0.10`）标出
两个块，都在 s18：

| 样本 | 块 | dark_frac | dark_cc_frac |
|---|---|---|---|
| s18 | 2 | 20.6% | 20.0% |
| s18 | 13 | 17.0% | 16.9% |

紧随其后没被标的是 s12q 块 9（9.9%）和 s10 块 24（6.7%）。被标出的块里细胞数也偏低，
方向和"洞吃掉细胞"一致。

结果写到 `qc_blocks.csv`，以及 xlsx 里的 `Block_QC`（长表）和 `Block_QC_Dark`
（块 × 样本的矩阵）两个 sheet。

`qc.action: report` 只报告，`drop_block` 把它从**所有**样本里剔除。没有逐样本剔除这个
选项：那样每个样本的块平均会算在不同的块集合上，两组均值不再可比。剔除依据是图像，
与分组标签无关，所以在看结果之前跑它不构成挑选。

### 这套东西修不了的

* **块里真的缺了组织**，只是现在**查得出来**（见上）。指标本身仍然不会自动修正，
  `CoveredDensity` 用的还是 brain mask 的覆盖率。
* **层。** 要求整个立方体落在 Isocortex 里面，就够不着上层——它们是薄薄一层外壳。
  这些块采的是**深层皮层**，主要是 5 和 6a。只发生在 1–4 层的效应在这里看不见，
  径向（软膜到白质）的迁移表型也不在这套块能测的范围内，只有切向梯度可测。
* **n。**

### 图

`<output.dir>/figures/` 下三种，和 `plot_bars.py` 共用 `plot_style.py` 的配色和
逐样本 marker，所以同一只动物在所有图上是同一个形状。

| 图 | 画什么 |
|---|---|
| `bars/primary_<指标>.png` | 一类一格，两根柱六个点：块平均的组间比较 |
| `bars/blocks_<类>_<指标>.png` | 28 个块并排（按前后排序），下面一条是该块的 log2fc |
| `forest/gradient_<类>_<指标>.png` | 逐块效应对三条解剖轴，看有没有梯度 |

图上不打星号。`ps.p_label` 印的是校正后的 p 和它来自哪个 family；六只动物的时候，
一颗星就是在请人把一根柱子当成结论。柱子上永远有逐样本的点：n=3 的柱加 SD 须就是
两个数字扮成一个分布。

## 按皮层层合并：`laminar.py`

放射状迁移决定的是细胞停在**哪一层**，不是哪个区。逐区的结果（0913_01）已经说明
Sox9+ 的变化是整个 isocortex 一起动，所以这里把 isocortex 所有区的同一层合并，再做
3v3。
...
分箱在 2026-09-14 定下，那时还没跑过任何按层的组间比较。

| 方案 | 箱 |
|---|---|
| `three_bin`（主分析） | upper = 1 + 2/3 + 4 ｜ L5 ｜ L6 = 6a + 6b |
| `two_bin` | superficial = 1 + 2/3 + 4 ｜ deep = 5 + 6a + 6b |

* **6b 并进 L6。** 图谱里只有约 57 µm 厚，83% 的体素离边界不到一个体素，比配准残差还薄。
* **L1 不单独成箱。** L1 和 L2/3 之间的边界是这批配准里最不可靠的线，两个方向都有问题：
  s10 部分皮层被压扁，L2/3 的细胞落进 L1 标签（占该样本 isocortex 细胞的 25.6%，
  沿原图 z 从 0.18 升到 0.52）；s12t 的 L1 标签落在软膜亮边上，几乎没有检出（4.6%）。
  其余四只在 11.5–15.7%。这个差距来自组织和检测，不是生物学。
* **L4 不单独成箱。** 43 个 isocortex 区里只有 25 个有 L4 标签。无颗粒区（MO、ACA、
  PL、ILA、ORB、AI、RSP、FRP、ECT、PERI）没有，所以合并后的"L4"只代表感觉皮层，
  而无颗粒区本该在 L4 位置的细胞已经算进了它们的 L2/3 和 L5。
* **L5/L6 这条线按可靠处理。** 两层都厚（约 270 / 500 µm），下面的胼胝体配得准。

每个方案必须把图谱在 root 下用到的**每一个**层标签都放进某个箱，否则直接报错。
漏掉一层会悄悄缩小所有分母。ACAv 的标签名里没有 "layer"（"Anterior cingulate area,
ventral part, 6a"），正则专门认这种写法。

### 指标

| 指标 | 算法 | 读法 |
|---|---|---|
| `LaminarShare` | 某类细胞在某箱的数 / 该类在所有箱的数（同一只动物） | **主指标。** 标记效率、剂量、脑大小都约掉，只剩深度。同一类的几个箱加起来是 1，一个箱升另一个就降，要一起读 |
| `BinComposition` | 某类在某箱的数 / 该箱所有标记细胞 | 比如 L5 里的 Sox9+ 占比。互补类（Sox9_pos / Sox9_neg）是同一个检验 |
| `Count` | 每箱原始计数 | 只作参考，带着标记效率（L1 细胞 7k–49k） |

比例给两种合并口径：

* `pooled`：各区计数先加总再相除，SS、MO 这种大区权重大。主口径。
* `area_mean`：每个区内先算比例，再对区取平均，每个区等权。任一只动物里细胞少于
  `area_min_cells` 的区整体剔除，保证六只平均的是同一组区；某区没有某箱的标签时，
  那个比例是**未定义**，不按 0 算。

不提供 Density：样本空间里合并的层带着约 20% 的脑大小差异，那已经被证明占了 Density
效应的一半。

校正 family 是 (scheme, pooling, metric, readout)，也就是同一个 readout 的几个箱，所以
m 只有 2–3。`role` 列标出 `primary`（three_bin × pooled × LaminarShare）、`secondary`、
`reference`（Count）。每行带逐样本值、置换排名，以及 `log2fc_without_<样本>`，专门用来
看 s10 这种单只动物撑不撑得住结果。
...
| 文件 | 内容 |
|---|---|
| `laminar_tests.csv` | 全部检验 |
| `laminar_per_sample.csv` | 每样本数值，画图直接取 |
| `laminar_token_qc.csv` | **原始层标签**上的细胞占比 + `unassigned`。薄层的配准问题在这里看，分箱之后就看不见了 |
| `laminar_layer_map.csv` | 每个层标签 → 区、层、各方案的箱，外加每只动物的细胞数 |
| `laminar_counts_long.csv` | 长表计数 |
...
写到 `<run>/figures/`：`00_layer_label_qc.png`（原始层标签占比，每只动物一根柱），以及
`<scheme>/<role>_<metric>_<pooling>.png`。每张图一行一个 readout、一列一个箱，同一行的
箱要一起读。每格和 `plot_bars.py` 一样：组均值 ± SD，六只动物各一个点，标 raw p 和
p_adj，下方写置换排名（x/10）；去掉任一只后方向会反的，也在下方注明。

## 一份 Excel：`summary_workbook.py`

`block_stats.py` 写九张彼此独立的表，`composition.py` 的比例还写在另一个文件里，
哪张表回答哪个问题没人说。这个脚本把一次跑的结果重排成**一个 Excel、三个问题**。

```bash
conda activate antsreg
python -m stats.summary_workbook \
    --run     /data/hdd12tb-1/fengyi/COMBINe/stats/0911_03_stats_roi_mid50 \
    --config  stats/configs/tsc_roi_mid50.yaml \
    --out-dir /data/hdd12tb-1/fengyi/COMBINe/stats/0912_01_summary_roi_mid50
```

结果目录按 `<日期>_<当日第几次>_<分析特征>` 命名，和 `block_stats` 的输出目录同一套
规律。里面是 `summary.xlsx` 加一份 `source.json`，记下它读的是哪次 block_stats、
用的哪个 config、多少个 box、怎么分的组——没有这个，一个按日期和特征命名的目录
三周之后没法复现。不给 `--out-dir` 就写回 `--run` 目录。

**输出文件里不出现中文。** 注释和终端不是交付物，xlsx 是，它要发给别人、进别的工具，
那些地方不一定渲染得出中文。这条是靠断言执行的，不是靠记性：写进 sheet 的每一个
字符串在落盘前过一遍 `_assert_ascii_text()`，有中文就报错退出，不会静默发出去。

它只读那次跑留下的 csv 和 `blocks_used.json`，不碰原图，所以对任何一次历史结果都能
补跑。`--config` 必须是**那次**用的 config：class_map 对不上会直接报错退出，不会
静默给出一个分母不对的比例。

| sheet | 回答 |
|---|---|
| `ReadMe` | 每个 sheet 和每个指标的含义、本次的 box 数 / 分组 / 方向约定 |
| `Q1_Group` | **每个样本先对所有 box 取平均，再组间比较。**n 仍是 3 vs 3 |
| `Q1_PerSample` | Q1 的每样本数值，一行一个 (readout, metric)，画图直接取 |
| `Q2_PerBox` | **同一位置的 box 组间比较**，一个 (box, readout, metric) 一个检验 |
| `Q2_PerBox_Values` | Q2 的原始数值 |
| `Q2_Spatial` | Share 的异质性 I² + 全局置换：细胞是不是换了地方 |
| `Q3_BoxVolume` | **同一位置的 box 在两组里组织体积是否相同** |
| `Q3_BoxVolume_PerSample` | 每 box 每样本的组织体积 / 组织占比 / 暗区占比 |
| `Blocks` | box 的图谱体积、中心坐标、主要脑区 |

**读的顺序是 Q3 → Q1 → Q2。** box 定义在图谱空间，体素集合逐样本完全一致，所以
图谱体积按构造相同；能不同的只有里面有多少是组织。Q3 不过关的 box，它在 Q1/Q2 里的
计数差异混着组织缺失，不是生物学差异。两种失败方式分开记：

* `fail_group_diff`：BH 之后的组间系统差异。
* `fail_spread`：六个样本里组织占比的 min/max 低于 `qc.volume_min_ratio`。抓的是
  单只撕裂的脑——那种情况会同时撑大它自己那组的方差，检验反而看不见，而这里一只脑
  就是一组的三分之一。阈值用的就是 `block_stats` 当时决定剔除哪些 box 的那个数，
  所以这张表和实际剔除的清单一致，不会互相打架；`in_stats` 列标明该 box 有没有
  进入 Q1/Q2。

稀柱那次的实测：32 根柱子里 4 根被标出（`25` s18 组织只剩 53%、`16` s18 76%、
`30` s8 84%、`3` s18 90%），全部是 `fail_spread`，没有一根是组间系统差异，
和 `block_stats` 剔除的 `[3, 16, 25, 30]` 完全一致。

比例给两种口径，它们回答的问题略有不同，在 box 之间细胞数悬殊时会分歧：

| | 算法 | 取舍 |
|---|---|---|
| `Fraction_pooled` | 所有 box 的计数先求和再相除 | 方差小，大 box 权重大。`composition.py` 用的就是这个 |
| `Fraction_boxmean` | 每个 box 内算比例再对 box 取平均 | 字面意义的"所有 box 取平均"，每个 box 等权，更吵，box 内无细胞时无定义 |

两者的分母都是**同一只动物的全部标记细胞**（class_map 的四个基础类之和），所以
标记效率、注射剂量、透明化质量都被约掉——这个设计里没有内参，成分比例是唯一能
组内归一的量。

只有一个 box 的跑法（例如 ROI 中段）里 `Q2` 是空的，写一行说明而不是留一张空表：
一个 box 没有"在哪儿"，同位置的组间比较就是 Q1，Share 恒等于 1.0。
