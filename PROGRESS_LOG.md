...
## 2026-09-06：纠正"门控让不校正变得正当"这个说法；深层改报集合层面的富集

用户问 L6-8 不做 FDR 有什么可解释性。查下来发现**我 0905 的说法过头了**。

**门控不控制任何错误率。** 它把 L7 的 family 从 98 缩到 33，但 33 个同时检验里即使
全是空的，也期望有 33×0.05 = 1.65 个 p<0.05。门控缩小的是搜索范围，不是错误率。
所以不校正的层**做不出逐区的推断性声明**。

**换成 FDR 也救不回来**：对门控之后的 family 做 BH，实测 L6 剩 1 / L7 剩 0 / L8 剩 0。
不是"FDR 太严"——L7 最小 p=0.00536，BH 首关需 ≤0.05/33=0.0015，**数据在那个深度
确实够不着**。所以 `none` 当初的理由（FDR 会全灭）是真的，但那不能推出"因此可以
不校正还照常声称"。

**能成立的是集合层面的结论**，已做成四列输出（每个 (class, metric, level) family 一组，
挂在该 family 每一行上）：`n_raw_sig_in_family` / `expected_false_in_family`（m×alpha）/
`family_enrichment_p`（单侧二项）/ `est_false_frac_in_family`（期望假阳 ÷ 实际命中）。
...
**深层的正确读法是两句话**：(1)「没有任何单个亚区达到校正后显著」；
(2)「但这批亚区整体富集远超偶然，约 14% 预期为假阳，按效应量排序靠前的最可能是真的」。
即深层回答的不是"哪个区显著"，而是"差异是弥散还是集中"以及"后续验证往哪儿做"。

`methods.md` 现在自动附一张集合富集表。README、config 注释、ReadMe sheet 的措辞
全部按上面改过（原来写的"门控是不校正的正当性来源"已删）。

**顺带确认**：病毒是**全身给药**，所以 s10 是左半球这件事对 reporter 阳性细胞密度
没有影响，那条待确认项关闭。
...
## 2026-09-06（续）：加了 moderated_t 作为可选检验；实测它在本项目尺寸下偏松

用户问深层有没有合适的统计方法。**深层的瓶颈不是多重检验，是每个检验只有 4 个
自由度**，所以有用的方法必须能跨脑区借统计量。

**新增 `stats.test: moderated_t`**（welch / student 之外的第三个选项，**默认仍是
welch，没有改用户还没跑过的原方案**）。经验贝叶斯方差收缩，Smyth (2004)：把同一
family 内各区方差往跨区拟合的先验收缩，实测 df 从 4 升到 7.5~20.5。
`variance_trend: true` 时先验随区均值变（limma-trend）。

**这是按论文重写的 numpy/scipy 实现，不是 limma 的封装。** Python 没有 limma 移植；
这台机器上也没有 R / rpy2，所以无法对拍。写 methods 引 Smyth (2004)，别写"用了 limma"。

**为什么必须要 trend**：细胞密度的残差方差随均值上升（L8 上 Spearman rho=+0.57）。
用全局先验时存活数随"要不要取 log"剧烈摆动（L7 上 11 vs 1）；开 trend 后两种尺度只
差一两个区（11 vs 9）。

**加了两道家族尺寸保护**：`<12` 个区退回 Welch，`<25` 不启用 trend。不加时 L2
（family 只有 8-9 个区）的先验 df 在 **0.6~49.1** 之间由噪声决定。回落记录在
`test` / `test_note` / `prior_df` 列，不静默。

### 自己验了一遍标定 —— 结论是它偏松，不能直接当确证

没有 R 可对拍，所以做了模拟验证（n=3v3 全空假设，1500 次重复）：

- **先验 d0 能正确估回**：真值 2 / 4 / 10 → 估计中位数 2.05 / 4.12 / 8.82。
- **边际标定正确**：p<0.05 比例 m=50 → 5.23%，m=200 → 5.01%，m=5000 → 5.01%
  （名义 5%）。**说明实现无误。**
- **但 BH 那一层不标定**：全空下 P(至少一个拒绝) 应 ≤5%，实测
  m=15 → 9.9%（trend 10.6%）、m=30 → 7.7%（trend **14.9%**）、m=50 → 6.5%
  （trend 11.3%）、m=200 → 5.3%。**偏松 1.3~3 倍。**
  原因：所有区共用同一个估计出来的先验，这是 BH 吸收不了的共模依赖；trend 多估一层
  结构所以更糟。
- **对照：Welch 在同样数据上边际只有 3.2~3.6%（名义 5%），是偏保守的。**

**所以两者是把真值夹在中间。** 本数据两者都配 bh 时的重叠：
L6 welch 1 / moderated 5 / 共有 **HIP**；L7 welch 2 / moderated 9 / 共有 **CA, DG**；
L8 welch 0 / moderated 2 / 共有 0。**读法：两者都有的当确证，只有 moderated_t
有的当线索。**

这些数字写进了 `moderated_ttest` 的 docstring、运行时 `[WARN]`、README，并用
`test_moderated_t_calibration` 钉住（断言边际在 0.03~0.08、且 BH FWER 仍高于名义——
若哪天不再偏松，那是 docstring 该改而不是断言该删）。

**过程中的一个教训**：我一度直接把 config 默认改成了 moderated_t + bh 并说它"更好"，
用户指出原方案还没跑过。已改回 welch + 原来的逐层校正，moderated_t 只作为可选项，
config 注释里给出四种组合的实测对比让用户自己选。**更重要的是，如果当时没做标定
模拟就推荐，会推荐一个 FWER 高 3 倍的方法。**

测试 27 → 29 项全绿。

---

## 2026-09-10：图谱空间小块采样 —— 取块方式，以及两次运行

起因是用户提出全脑分区比较不公平：透明化会缩水、开裂，样本也不是每个都是完整半脑。
想改成在图谱空间随机取若干小块，把同一套块 apply 到所有样本再比。

### 为什么值得做：Density 显著项有一半是分母

先用已有结果验了这个直觉，**它是对的**。`0908_marker_ungated` 的 6 个 Density 显著项
拆开是 `log2fc(Density) = log2fc(Count) − log2fc(Volume)`，两项各占一半：

| 指标 | log2fc | Hedges g | 校正后显著 |
|---|---|---|---|
| Count | +0.24 ~ +0.60 | 0.55 ~ 1.75 | 否 |
| Volume | −0.19 ~ −0.28 | −0.65 ~ −0.87 | 否 |
| Density | +0.50 ~ +0.80 | 4.4 ~ 7.8 | 是 |

全脑体积（sample 空间，mm³）：s10 64.8 / s18 59.1 / s11 82.4 ‖ s8 57.4 / s12q 71.2 /
s12t 49.1，最大比最小 1.68 倍。level 2-3 的 28 个结构里 **26 个**实验组更小（13~23%），
但 RelativeVolume 围零散布 —— 是全局尺度差异，不是局部萎缩，跟缩水无法区分。

图谱空间的块体积对所有样本是同一个常数，除以它不改变任何统计量，所以这个指标在数学上
就是「固定解剖 territory 里的计数」，缩水项被消掉。**预期结论会落到 Count 一侧。**

### 取块方式

工具在 `../Registration_toolkit/tools/pick_blocks.py`，配置
`configs/pick_blocks.example.yaml`。这边只读它产出的 `blocks.json`，不重新布点。

**1. 块定义在原生图谱网格上，不是各样本的变体网格。**
六个样本的「图谱空间」不是同一个网格：s10 是左半脑，orientation `[-1, 3, -2]`、slicing
`[285,514]/[44,365]`、形状 `(252,574,321)`；其余五个是 `[1, 3, 2]`、`[285,510]/[35,356]`、
`(249,572,321)`。

**第一次跑时 57 个块被全部剔掉**：原生右半球的块活不过 s10 的镜像裁剪，左半球的块活不过
另外五个，交集为空。修法是对镜像样本先把块体积沿左右轴翻转再推过去。左右轴由「两半镜像
吻合度」推出来（实测 99%），不写死。

**2. 坐标用管线自己的函数推，不自己写逆变换。**
`reorient_volume` / `_parse_slicing` / `background_pad_width` 直接作用在一个块 id 体积上，
每个样本推一遍，然后检查每个块 id 的体素数是不是还等于 `block_vox³`，不等就说明被裁到了，
整块剔除。这一步顺带把半球问题也解决了。手算 DV 位置我差了一格，推体积的结果是对的。

**3. 为什么是立方块，为什么是 600 µm** —— 都是量出来的，不是拍的：

| 结构 | 最大内切球半径 | 区内中位 | 800 µm 立方块可放位置 |
|---|---|---|---|
| Isocortex | 649 µm | 200 µm | 0 |
| Hippocampal formation | 590 µm | 141 µm | 0 |
| Caudoputamen | 656 µm | 166 µm | 0 |

大平板也不行，**限制是曲率不是厚度**：1600×1600×300 只剩 1163 个位置，2000×2000×300 为 0。
长方体能换单块体积但代价是块数：1200×1200×300 是 0.43 mm³ 只有 7 个不重叠位置，
600 µm 立方是 0.216 mm³ 有 27 个。这笔交换不值 —— 每块细胞数从来不是瓶颈（500 µm 立方
已有约 900 个细胞，泊松 CV 3%），而在某个方向薄的块正好在配准误差最伤的方向上短
（误差 100~250 µm，300 µm 厚会错位三分之一到八成，600 µm 立方只错位六分之一到四成）。

**4. `min_surface_dist_um` 在严格纯度下是摆设**，设 0 和设 200 的合法中心数完全一样；
`avoid_dorsal_um: 400` 也只去掉几百个体素。纯度约束本身已经把块推离脑表面了。

**5. `spacing_scale` 是真正要调的那个**。默认 1.0（刚好不重叠）太紧：28 个块的最近邻
距离中位 712 µm，而块边长 600 µm，**13 个基本贴着**，相邻块共享同一片组织和同一份配准
误差。改成 2.0 之后 8 个块，最近邻中位 1413 µm，无贴合。

### 必须写进 methods 的两条限制

**块只采得到深层皮层。** 28 个块里 layer 5 占 43%、layer 6a 占 42%，layer 4 为 0。
这是几何决定的：上层皮层是一层薄壳，纯立方块挤不进去，换 400 µm 立方也只是变成 55/38。
**效应若在 layer 2/3，这套分析看不见。**

**coverage 测不出破损。** `*_in_atlas.nii.gz` 的 brain mask warp 到图谱空间后覆盖
Isocortex 的 99.9%，六个样本 28 个块的 coverage 全是 1.0000，所以 CoveredDensity 和
Count 的 p、g 一模一样。它是个包络，看不见裂缝。真正记了破损的是 guide mask 的
`damage_labels`，但只有 s11（13.6%）和 s12q（1.5%）画了。找坏块只能看逐块计数的离群，
例如 11 号块 s18 只有 105 个细胞而 s10 有 547 个。

### 统计：`stats/block_stats.py`

Welch + BH，分类沿用 MADM 的 `class_map`（4 个互斥基础类 + 4 个重叠聚合类）。两个家族：

- **Primary**：每样本先对所有块取平均，一个 (类, 指标) 一个检验。**n 仍然是 3 对 3** ——
  块被平均进样本之后才检验，任何地方都没有把块当重复。
- **Per_block**：一个 (块, 类) 一个检验，n=3v3，在每个 (类, 指标) 内 BH。exploratory。

**加了留一法，这在 3v3 下不是可选项**。一个样本就是一组的三分之一。

### 两次运行的结果

| | 第一次 | 第二次 |
|---|---|---|
| spacing_scale / seed | 1.0 / 0 | 2.0 / 1 |
| 块数 | 28 | 8 |
| 最近邻中位 | 712 µm | 1413 µm |
| Primary 显著 | 0 / 24 | 0 / 24 |
| per-block 显著 | 0 / 672 | 0 / 192 |
| per-block 名义 p<0.05 vs 随机期望 | 3 vs 11.2 | 2 vs 3.2 |

**两次都是零显著，而且 per-block 的名义显著数都在随机期望或以下。**

留一法：**八个类没有一个扛得住去掉任意一个样本**。第一次剩余比例全部低于 0.4，四个变号。
正向效应（MADM_all、non_glia_MADM）靠 s18（碎裂那个，落在块里的细胞数六个里最低）；
Sox9 相关的负向效应靠 s11 和 s12q。

第二次（独立块）排序变了：三个 Sox9 类全部下降，效应量翻倍（Sox9_pos log2fc −0.41，
g −0.78，p 0.297），留一法剩余比例从第一次的 −0.45/−0.31/−1.14 变成 0.51/0.55/0.34。
但逐块看就散了 —— 8 个独立块里 Sox9_pos 只有 5 降 3 升，组内散布比组间大
（3 号块对照 140/66/219，实验 71/172/27）。

**能说的是：Sox9 阳性细胞在深层皮层偏低是这批数据里最稳的方向，但达不到显著，也没有
空间一致性支撑。当成扩样本后要专门检验的定向先验，不是结果。**

### 一条留给以后的线索

第一次的 2 号块（前扣带皮层背侧部 6a/5 + MOs 6a，中心 6.06/2.30/4.12 mm）的 glia_MADM：
对照 69 / 70 / 68，实验 26 / 30 / 18。完全分离，六次留一法全部保持（p 0.004~0.082），
原始 p 0.0053，g −8.1，但 224 个检验的家族里 p_adj = 0.148。回查原分区分析，ACA 各子区的
glia_MADM 几乎全部同方向下降（log2fc −0.29 ~ −1.37），全不显著，方向对得上。

**整个家族的名义显著数低于随机期望，所以这是假设不是结果。** 第二次抽的 8 个块没有落在
ACA 的，所以这条线索既没被验证也没被推翻。

### 文件

```
../Registration_toolkit/tools/pick_blocks.py        布点 + 画图 + --napari
../Registration_toolkit/configs/pick_blocks.example.yaml
stats/block_stats.py                                统计
stats/configs/tsc_blocks.example.yaml               第一次（28 块）
stats/configs/tsc_blocks_sep2.yaml                  第二次（8 块）
```

输出（`/data/hdd12tb-1/fengyi/COMBINe/stats/`）：

```
blocks_ctx600/            blocks.json + blocks.csv + 两张图谱切面 PNG
blocks_ctx600_sep2/       同上，第二次
0910_blocks_ctx600/       block_stats.xlsx（6 sheet）+ 三份 csv + blocks_used.json
0910_blocks_ctx600_sep2/  同上，第二次
```

每个结果目录里都带着自己那份 `blocks_used.json`，两次可以各自复现。**两次都要报**，
说清楚哪次是 28 个含相邻块、哪次是 8 个独立块。

### 下一步

- 块的位置必须在看到分组结果之前固定。已经看过 28 块的结果了，所以再缩小家族就是事后
  调参 —— 第二次是按「块不独立」这个跟结果无关的理由改的，这一点要在 methods 里说明。
- 想覆盖上层皮层只能用沿皮层法线的薄板，但以现在 100~250 µm 的配准误差分不干净层，
  暂不做。
- ACA 的 glia_MADM 下降，等扩样本之后当单一定向假设检验。

---

## 2026-09-15：普通免疫荧光矢状切片的 2D 配准流程（section2d）

**做了什么**：
- 新增 `src/registration_ants/section2d.py` + `scripts/register_sections_2d.py` +
  `configs/sections2d.example.yaml` + `tests/test_section2d_smoke.py`。
  流程：朝向（旋转/镜像）→ 候选矢状面网格（距中线 × yaw × roll）上各跑 Similarity、按 MI 打分
  → 细网格 → pattern search 精修 → 选定平面上 Affine → SyN → 标签回贴切片、细胞映射到 3D 图谱坐标。
- `config.resolve_atlas_preset` 从 `load_config` 里抽出来共用；`transforms.transform_cell_points` 加了 `dim`。

**关键决定**（都在 DevCCF P04 合成切片上实测：斜面不在网格上、非单调对比度、偏场、≤100 µm 局部形变、撕裂 + damage mask、缩放 0.92、旋转 17°）：
- 搜索用 Similarity 不用 Affine：外侧矢状面更小，单一缩放 size_ratio 本身区分平面；Affine 会把差异吸收掉。
- **只靠网格不够**：真值平面在所有打分设置（40/20 µm × Similarity/Affine × MI/CC）下都是最优，
  但网格停在 1400/5/−2.5（真值 1500/3.3/−6.2），细胞 3D 误差中位数 160 µm，Affine 和 SyN 一样
  —— 面外误差 2D 配准救不回来。打分的坑很窄而且 ML 和 roll 耦合（离真值 20 µm/1.2° 的点比错误答案还差）。
  加 26 邻域 pattern search 后：1500 → 1500/3.1/−6.9，400 → 380/3.1/−6.25。
- **SyN 默认 mattes 不用 CC**：细胞 3D 误差中位数（ml 400 / 1500）Affine 26/18 µm，SyN+CC 38/34 µm（比不做还差），
  SyN+mattes 26/18 µm、面内误差 10~12 µm。合成对比度是自己编的，真实数据仍要看 qc.png 判断 SyN 值不值得做。
- 靠近中线时位置本身就模糊：ml 400 的 far_gap 只有 0.028（ml 1500 是 0.077），ml 0 打分几乎一样。

**遇到的问题**：
- ANTs 复现：搜索在 worker 里（单线程 + 种子）两次逐位一致，但主进程里的 Affine/SyN 不一致 ——
  ITK 线程数在 import 后只读一次，`set_ants_deterministic` 设得太晚。CLI 顶部 import 前设
  `ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1` 后两次全部一致。
- spawn 进程池：调用脚本没有 `if __name__ == "__main__":` 时 worker 会重跑整个脚本然后死掉，父进程**静默卡死**（不报错）。已写进 docstring 和 README。
- 图谱外缘的候选面（组织 1892 px）Similarity 发散后，Mattes 打分抛 ITK "Zero-valued spacing"，
  一个候选就让整个搜索崩掉；改为该候选记 inf 分并记录 error。

**下一步**：
- 拿真实切片跑，先看 qc.png 的打分曲线是否单峰、size_ratio 是否合理（图谱年龄要和切片匹配）。
- 同一只动物的连续切片可以加联合约束（共享倾角、ML 按切片间距单调），`search_candidates.csv` 里已保留全部候选分数。

### 续：DeMBA P5、输入格式诊断、默认朝向

- **DeMBA P5**：`DeMBA_P5_reference.tif` 读进来轴0 = 左右（镜像重合 0.986）、轴1 = 上→下、轴2 = 前→后，
  规范方向要 `orientation: [1, 3, 2]`（写在 2D 配置里，不进共用预设）。同一套合成切片：ml 1500 → 1490/3.1/−6.25，
  细胞误差中位数 17 µm、脑区一致 90.6%；ml 400 → 380/2.5/−6.25，44 µm、80%（靠中线，far_gap 0.041）。
- **输入可能是多通道灰度（CYX，8 bit）、RGB 融合图（YXS）或带 Z 层**：新增 `section_io.py`，按 tifffile 的 series axes
  判断，不按形状猜 —— 3 个平面的 8-bit 文件可能是 RGB 也可能是 3 个灰度通道，处理方式相反。
  `scripts/inspect_sections.py` 出属性报告 + 每个平面的缩略图。
- 在 `/data/hdd12tb-1/fengyi/COMBINe/MADM/data_for_inference/troy/` 5 张（P30、RHem，只拿来测工具）上：
  全是 Photoshop 存的 8-bit RGB，**分辨率是 72 dpi —— 编辑器默认值，不是标定**，工具报 UNKNOWN 而不是换算成 352.8 µm/px。
  颜色组合（每个平面最亮 5%）：B 单独 41%、R+G 26%、R 15%、G 15%、B 与 R/G 同亮 ≤2% —— 蓝色只有核染色，
  这批图 `channel: 2`（或 DAPI=[0,0,255] 解混）就是干净的 DAPI。
  最初用背景 99 分位当"亮"的阈值（只有 2~9 灰度级）时 >90% 组织像素算成 R+G+B，没有信息，已改。
  RGB 不再给"建议平面"：按覆盖度排序在两张图上分别选了 B 和 G，单个颜色平面本来就混着 marker。
- **默认朝向 anterior 左 / dorsal 上**：给提示时另外 7 种 90° 旋转/镜像也各试一次；打分更好的不是提示朝向时照用并
  WARNING，`plane.json` 记 `orientation_matches_hint`。合成切片（实际镜像 + 转 110°）被正确标出，平面照样找对。

---

## 2026-09-15（续）：isocortex 按层结果可视化 + L1 伪差定位（s10 / s12t）+ 皮层层次校准方案

**做了什么**：
- 新增 `stats/plot_laminar.py`（README 已加一节），读 `laminar.py` 的输出目录画柱状图：
  ```bash
  python -m stats.plot_laminar --run /data/hdd12tb-1/fengyi/COMBINe/stats/0914_03_laminar_iso
  ```
  输出在 `<run>/figures/`：`00_layer_label_qc.png`（原始层标签占比，每只动物一根柱），以及
  `<scheme>/<role>_<metric>_<pooling>.png`，共 11 张 + PDF。每张图一行一个 readout、一列一个箱，
  每格组均值 ± SD + 六个样本点 + raw p / p_adj，下面标置换排名；去掉任一只方向会反的也注明。复用 `plot_bars.draw_panel`。
- 确认统计里 s12t 用的是 `TSC_ants/s12t/DeMBA_0828`（s12t 最新的一次）。能直接查到 s12t 路径的 9 次 stats run 全部是这个版本；
  0908、0908_2 和 0910/0911 的 blocks 系列没查到，未核对。
- 逐项查了 s10 / s12t 的 L1 问题（脚本在当次 scratchpad，未入库）。

**关键结果**：
- 0914_03 主分析（three_bin × pooled × LaminarShare）没有一项过 BH。"KO 偏深层"的趋势来自 s10 和 s12t 的层标签错位，不可解释。
  two_bin 的 deep 占比同样受影响（MADM_all deep：s10 0.448 最低、s12t 0.619 最高，其余四只 0.49~0.52；去掉 s12t 后 log2fc 从 0.19 降到 0.10）。
- **s10**（Ctrl）：用户确认处理时皮层被压扁。L1 标签上的细胞占 25.5%，其余四只均值 12.5%；
  L2/3+4 −6.6、L5 −1.9、L6 −4.1 个百分点，也就是整个深度轴往浅层错，不只是 L2~4 挤进 L1。L1/L2-3 细胞密度比 1.43。
- **s12t**（KO）：用户肉眼看整体配准不错，样本最小（affine det 0.496，六只里最小）。
  - L1 占 4.5%，多出来的份额在 L5 +7.3、L6 +3.7，L2/3 没有增加，和 s10 正好镜像。
  - **不是 mask 画出了组织**：手绘皮层 guide 外缘在组织内侧 58 µm（s8 为 65 µm），画进背景 >26 µm 的比例两者都是 ~3%。
  - L1 标签确实铺在组织上（占 isocortex 12.9%，和其他样本一样），距组织表面的深度和 s8 一样（p50 45 vs 49 µm）。
  - **差别在细胞**：表层 40–80 µm 的细胞密度 s12t 4,379/mm³，s8 13,439；s12t 的致密带晚约 40 µm 才开始，
    所以 L1 标签上几乎没有细胞（L1/L2-3 密度比 0.26，s8/s12q 约 0.6）。分不清是表层漏检还是样本小导致层相对靠里。
  - 40 µm 解释不了 L5/L6 多出的份额，修好 L1 深层偏多也不会消失。
- **上层 Sox9+ 占比不受 L1 界线影响，结果成立**（two_bin superficial = L1+2/3+4，BinComposition pooled，预先定义的次要分析）：
  s10 25.7 / s18 26.7 / s11 23.6 ‖ s8 22.7 / s12q 19.8 / s12t 18.1（%）；Ctrl 25.3、KO 20.2，g −2.05，raw p 0.042，p_adj 0.083，
  两组完全分开（置换排名 1/10）。
  - 原因：L1 本身的 Sox9+ 占比两组一样（27.2 vs 27.4），在 L1 和 L2/3 之间错分的细胞都还留在上层，组成不变。
  - 内部核对（不作为方案）：只算 L2/3+4 时 g −2.59，仍然完全分开，说明不依赖 L1 这条界线。
  - 还不能说是上层特有：L5 g −1.00、L6 g −1.20 方向一样，只是没分开。

**关键决定**：
- **上层 = L1 + 2/3 + 4，必须包含 L1。** 用户明确反对去掉 L1：L1 细胞多，是生物学目的的一部分。不给 `laminar.py` 加 exclude 选项。
- **不再在配准上校准层边界**：配准通道（2.6×2.6×32 µm）只看得到软膜和白质两条线，层边界没有对比度；
  偏差约 2 个 20 µm 体素，在 ANTs 逐次运行的噪声范围内；改 guide mask 也动不了细胞从哪里开始。
- 层分布（LaminarShare）读数按"未检测到变化，表观偏深由 s10/s12t 的层标签错位承担"报；上层组成是目前可以用的上层结果。

**皮层层次校准方案（待 730 通道）**：
- 730 通道是 Sox9 核染色（不是所有细胞都有，但密度大，信号略弱）。原始 0.65×0.65×8 µm，用户正在拼接，降采样到 2.6×2.6×32。
- **只用它定软膜和白质两条边界**，每个细胞算相对深度（0 = 软膜，1 = 白质），再按图谱层厚比例分层。
  不用 Sox9 密度定 L1/L2-3 这类层内界线：P5 星形胶质的层结构弱，而且要检验的正是 KO 的 Sox9+ 占比，用它定边界是循环论证。
- 注意：水平切面成像下，背侧皮层的深度方向沿 z，降采样后只有 32 µm，定不准；侧面皮层的深度方向在平面内（2.6 µm）。

**下一步**：
- 730 大图拼好后：先核对它和 `registration.tif` 同一网格（尺寸、原点、裁剪、翻转），否则配准变换和细胞坐标没法直接套用。
- 预实验：在 s8（配准好）、s12t、s10 上沿皮层深度画 Sox9 剖面。背侧皮层在原分辨率小块上做；只有降采样大图时先做侧面皮层。
  看两件事：软膜下 Sox9 带和白质边界能否稳定识别；按两条边界算的相对深度能否把 s12t 的约 40 µm 偏移和 s10 的挤压拉回一致。
  同时检查两组皮层厚度有没有系统差异。
...
**顺带纠正一个对 `syn_unlabelled_pct` 的误读（重要）**：
- 我一开始按这个指标排序，把 s022（26%）当成全批最差，用户指出配准看着挺好 —— 用户是对的。
- 这个指标的分母是【自动 Otsu 组织 mask】，不是真正的脑组织。s022 的 qc.png 左上角能直接看到：
  青色 mask 圈进了亮脑组织外面一大圈空白。
- 量化（"暗" = 比有标签脑组织的第 5 百分位还暗，即 mask 扫进来的背景）：
  s022 的"未标注组织"里 **99.7% 是背景**，真正没盖到的脑组织只有 **0.1%**，按真实覆盖算是全批最好的之一。
  s023 同理（97.3% → 0.1%）、s017（71% → 2.3%）、s016（91.5% → 0.2%）、s019（82% → 1.0%）。
- 真正还剩余量的是 s015（8.7%）、s021（8.8%）、s020（8.4%）—— 但也可能是切片带到了图谱平面之外的
  组织，未验证，不下结论。
- 另一个说错的：s022 那块"浮在黑背景上的小脑" —— 看 qc 的 atlas plane 面板，ml 3570 µm 这个矢状面上
  图谱【本身】就带着两块分离的小结构；样本这一刀没有那块组织而已。s022 的 `syn_outside_pct` 只有 5.95%，
  就在全批中位数 5.10 附近。
- **结论：排 `syn_unlabelled_pct` 等于排组织 mask 画得松不松，不是排配准质量。**
  要按配准质量排，得先把 mask 内的暗背景剔掉（检查脚本在当次 scratchpad，未入库）。

**下一步（可选）**：
- 脑区太细时用 `--level 6` 折叠到大结构，填色图会好读很多（`atlas_utils.collapse_labels_to_level`）。
- s017/s020/s021/s022 这几张最外侧的切片值得回头看配准本身。
- 可以考虑给 `_coverage()` 加一个剔掉暗背景的 unlabelled 指标（或者收紧 `tissue_threshold`），
  否则 summary.csv 这一列会一直误导排序。
